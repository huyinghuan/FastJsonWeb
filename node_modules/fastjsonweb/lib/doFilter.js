...
function doFirst(request,response){
  ...
  if(router.isStaticResorce(pathname)){
      var action = new StaticResource();
      action.execute(request,response);
      return;
  }
  doMain(request,response,pathname);
}

function doMain(request,response,pathname){
    doLast(request,response,pathname);
}

function doLast(request,response,pathname){
   var postParameter='';
   
   request.on('data', function(chunk){
		console.log("执行on data");
   ...
   request.on('end',function(){    
      ...
      response.write("postParameter = "+JSON.stringify(postParameter));
   ...
   //   var Action = router.route(pathname); //根据url 获取映射 处理Action
      router.route(pathname); //根据url 获取映射 处理Action
      /*
    ...
    //  response.writeHead(200, { 'Content-Type': 'x-application/json'}); 
      execute.call(parameterObj,request,response);//call函数 设置 方法的上下文
	  */
	  response.write("urlParameter = "+JSON.stringify(urlParameter));
	  response.end();
    });
	
}
exports.doFilter = doFirst;
